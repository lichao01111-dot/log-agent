...
| 阅读目标 | 源码 | 对应测试 |
|---|---|---|
| 搜索与推理的应用端口、规范化 DTO、错误类型 | [`application/ports.py`](../src/log_agent/application/ports.py) | [`test_ports.py`](../tests/unit/application/test_ports.py) |
| 更底层的结构化模型 Client 与文本 Sanitizer 契约 | [`application/model_ports.py`](../src/log_agent/application/model_ports.py) | [`test_model_ports.py`](../tests/unit/application/test_model_ports.py) |
| Port 的确定性内存实现 | [`adapters/fakes.py`](../src/log_agent/adapters/fakes.py) | [`test_executor.py`](../tests/unit/application/test_executor.py) |
| 应用如何消费 Port | [`application/executor.py`](../src/log_agent/application/executor.py) | [`test_fake_investigation.py`](../tests/integration/test_fake_investigation.py) |

阅读时先看 `SearchRequest` 和 `SearchResult` 的 `__post_init__()`，再看 `LogSearchPort` / `ReasoningPort` 的 `Protocol` 方法，最后去 Fake 中观察一个 Adapter 如何满足这些协议。

这几层代码各自负责不同的约束：

```text
Protocol             规定调用方向和方法签名
Request / Result DTO 在进程边界校验形状、数量和引用关系
Adapter              把外部 wire 数据转换为 DTO
CommandExecutor      校验这份结果是否属于当前 Command
State machine        再校验它能否改变领域状态
```

`Protocol` 本身不会阻止一个恶意或错误实现；真正的防线来自 DTO 构造校验、Executor 的关联校验以及状态机不变量。`PortError.safe_message` 也只是要求 Adapter 提供安全文本，并不会自动脱敏原始异常。
