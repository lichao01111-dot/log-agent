...
[护栏复盘](第7章-护栏预算与终止.md)说明预算和终止如何分布在当前实现中；主线[第七课](07-真实适配器与脱敏边界.md)定义真实接入闸门，第八章定义评估门槛。本章把这些能力放入一个运行时，但不会重新定义阶段函数、状态转移或判断规则。
...
| 应用层 scope 授权和模板标识 | 已完成（非执行计划） | `SafeQueryPipeline`；真实 Renderer 与服务端只读 RBAC 待第 7 章实现和验收 |
...
| 安全 trace 与运维指标 | 未完成 | 第十章给出设计和验收条件 |
| 上线质量门 | 未完成 | 需要第八章评估集和真实 Adapter 测试通过 |
...
> Runner 已经存在；第九章的任务不是重写循环，而是建立唯一 composition root 和安全用户入口。
