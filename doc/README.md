...
| 5 | [领域知识配置](05-领域知识配置.md) | 严格 JSON、KnowledgeSnapshot、权限隔离 | ✅ Schema/Loader 已实现 |
| 6 | [知识投影与结构化推理](06-知识投影与结构化推理.md) | KnowledgeProjection、模型草稿边界、证据别名 | ✅ provider-neutral + Fake 已实现 |
| 7 | [真实适配器与脱敏边界](07-真实适配器与脱敏边界.md) | Splunk/模型 wire contract、Redactor、契约测试门槛 | 🔴 文档设计；等待真实 MCP 与生产策略 |
| 8 | [评估与回归](第8章-评估.md) | incident 数据契约、结果/过程指标、发布门槛 | 🟡 确定性测试已有；评估集与 harness 待实现 |
| 9 | [Agent Loop 装配与安全入口](第9章-AgentLoop骨架实现.md) | Composition Root、入口授权、运行恢复边界 | 🟡 Runner 已有；生产入口/持久化待实现 |
| 10 | [可观测性与迭代](第10章-可观测性与迭代.md) | 安全 Trace、指标、回放、评估反馈闭环 | 🔴 文档设计；Trace sink 待实现 |

完成第 10 章后，再阅读 [Capstone：迁移与扩展](Capstone-迁移与扩展.md)。四阶段结构适用于诊断/调查类任务，不是所有 Agent 的固定模板。

## 第 7–10 章如何融入现有实现

教学阅读顺序是 7 → 8 → 9 → 10，但工程实施不是“写完一章再永远关闭它”：
...
- 第 7 章不改领域状态机，而是把现有 Port 映射到真实 wire contract。
- 第 8 章既评最终诊断，也把权限、预算、证据和泄漏测试设为第 7、9、10 章的发布门。
- 第 9 章复用现有 Runner；新增的是身份授权、装配、幂等、持久化和恢复边界。
- 第 10 章只记录安全控制事实，并把经过审批、脱敏的失败案例送回第 8 章，不直接改 Prompt 或知识。

因此，最合理的下一步不是立刻写四个新模块，而是先取得第 7 章列出的真实契约，同时并行实现第 8 章中不依赖生产数据的 deterministic eval harness。
...
- [护栏、预算与终止复盘](第7章-护栏预算与终止.md)：把旧版第 7 章映射到当前状态机与查询安全实现；它是补充专题，不是另一条课程主线。
